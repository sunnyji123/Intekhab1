...
if st.button("Generate Realistic Video ✨"):
    if not HEYGEN_API_KEY:
        ...
        try:
            ...
            upload_url = "https://upload.heygen.com/v1/asset/upload"
            headers_upload = {"X-Api-Key": HEYGEN_API_KEY}
            
            upload_response = requests.post(upload_url, files=files, headers=headers_upload)
            
            if upload_response.status_code == 200:
                image_id = upload_response.json().get("data", {}).get("id")
                ...
                st.error(f"Image Upload Error: {upload_response.text}")
